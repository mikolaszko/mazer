...
Background
~~~~~~~~~~
I really like perfect algorithms. This one shows how to generate a maze that
guarantees all cells can reach all other cells, it just may take some time to
get there. I introduce stacks, and show how the algorithm generates the maze
visually.
...
public:
  OneLoneCoder_Maze() { m_sAppName = L"MAZE"; }
...
protected:
  ...
  virtual bool OnUserUpdate(float fElapsedTime) {
    ...
    auto offset = [&](int x, int y) {
      return (m_stack.top().second + y) * m_nMazeWidth +
             (m_stack.top().first + x);
    };
    ...
    if (m_nVisitedCells < m_nMazeWidth * m_nMazeHeight) {
      ...
      // East neighbour
      if (m_stack.top().first < m_nMazeWidth - 1 &&
          (m_maze[offset(1, 0)] & CELL_VISITED) == 0)
        neighbours.push_back(1);
      // South neighbour
      if (m_stack.top().second < m_nMazeHeight - 1 &&
          (m_maze[offset(0, 1)] & CELL_VISITED) == 0)
      ...
      if (!neighbours.empty()) {
        ...
        case 0: // North
          ...
          m_stack.push(
              make_pair((m_stack.top().first + 0), (m_stack.top().second - 1)));
          break;
        ...
        case 1: // East
          ...
          m_stack.push(
              make_pair((m_stack.top().first + 1), (m_stack.top().second + 0)));
          break;
        ...
        for (int py = 0; py < m_nPathWidth; py++)
          for (int px = 0; px < m_nPathWidth; px++) {
            if (m_maze[y * m_nMazeWidth + x] & CELL_VISITED)
              Draw(x * (m_nPathWidth + 1) + px, y * (m_nPathWidth + 1) + py,
                   PIXEL_SOLID, FG_WHITE); // Draw Cell
            else
              Draw(x * (m_nPathWidth + 1) + px, y * (m_nPathWidth + 1) + py,
                   PIXEL_SOLID, FG_BLUE); // Draw Cell
          }

        // Draw passageways between cells
        for (int p = 0; p < m_nPathWidth; p++) {
          if (m_maze[y * m_nMazeWidth + x] & CELL_PATH_S)
            Draw(x * (m_nPathWidth + 1) + p,
                 y * (m_nPathWidth + 1) + m_nPathWidth); // Draw South Passage

          if (m_maze[y * m_nMazeWidth + x] & CELL_PATH_E)
            Draw(x * (m_nPathWidth + 1) + m_nPathWidth,
                 y * (m_nPathWidth + 1) + p); // Draw East Passage
        }
      }
    }
    ...
    for (int py = 0; py < m_nPathWidth; py++)
      for (int px = 0; px < m_nPathWidth; px++)
        Draw(m_stack.top().first * (m_nPathWidth + 1) + px,
             m_stack.top().second * (m_nPathWidth + 1) + py, 0x2588,
             FG_GREEN); // Draw Cell
